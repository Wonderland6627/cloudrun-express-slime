...
**⚠️ 重要提示：Git部署模式默认不会自动部署**

- **默认行为**：代码提交后**不会自动生效**，需要手动触发部署
- **手动部署**：在云托管控制台，进入服务详情页 → 点击 **"新建版本"** → 选择相同的Git仓库和分支 → 点击 **"部署"**
- **自动部署**：可以配置Webhook实现代码提交后自动部署（见下方说明）
...
**方式二：配置自动部署（推荐）**
1. 在云托管服务详情页，找到 **"自动部署"** 或 **"Webhook"** 设置
2. 复制云托管提供的Webhook URL
3. 在Git仓库设置中添加Webhook：
   - GitHub: Settings → Webhooks → Add webhook
   - Gitee: 设置 → WebHooks → 添加WebHook
   - GitLab: Settings → Webhooks → Add webhook
4. 将复制的URL粘贴到Webhook配置中
5. 选择触发事件：`push`（代码推送时触发）
6. 选择分支：通常选择 `main` 或 `master`
7. 保存配置

配置完成后，每次向指定分支推送代码时，云托管会自动触发部署。

**⚠️ 注意事项**：
- 首次配置自动部署建议在测试环境验证
- 确保Git仓库的Webhook权限已正确配置
- 自动部署会消耗构建资源，注意控制频率
